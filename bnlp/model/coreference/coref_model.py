...
import bnlp.data.conll2012 as conll
import bnlp.metric.evaluator as metrics
import bnlp.model.coreference.coref_ops as coref_ops
import bnlp.utils.text_tool as text_tool
import bnlp.utils.cal_tool as util
from bnlp.data.word2vec import EmbeddingDictionary
...
class CorefModel(object):
    def __init__(self, config):
        ...
        self.head_embeddings = EmbeddingDictionary(config["head_embeddings"],
                                                   maybe_cache=self.context_embeddings)
        self.char_embedding_size = config["char_embedding_size"]
        self.char_dict = text_tool.load_char_dict(os.path.join(project_dir, config["char_vocab_path"]))
        self.max_span_width = config["max_span_width"]
        self.genres = {g: i for i, g in enumerate(config["genres"])}
        if config["lm_path"]:
            self.lm_file = h5py.File(os.path.join(project_dir, self.config["lm_path"]), "r")
        ...
        input_props.append((tf.float32, [None, None, config["lm_size"], config["lm_layers"]]))  # Tokens_lm_emb. 仅用于中文
    ...
    def start_enqueue_thread(self, session):
        with open(os.path.join(project_dir, self.config["train_path"])) as f:
    ...
    def restore(self, session):
        ...
        checkpoint_path = os.path.join(project_dir, 'data', self.config["log_dir"], "model.max.ckpt")
    ...
    def tensorize_example(self, example, is_training):
        ...
        for i, sentence in enumerate(sentences):
            for j, word in enumerate(sentence):
                ...
                char_index[i, j, :len(word)] = [self.char_dict[c] for c in word]  # char_dict的value是int
        ...
        if "sentences_lm_emb" in example:
            sentences_lm_emb = example["sentences_lm_emb"]
        else:
            sentences_lm_emb = np.zeros(lm_emb.shape)

        example_tensors = (
            tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre,
    ...
    def truncate_example(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids,
                         genre, is_training, gold_starts, gold_ends, cluster_ids, tokens_lm):
        ...
        return tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, tokens_lm
    ...
    def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len,
                                 speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, tokens_lm):
        ...
        if not self.lm_file:
            # 采用Word2Vec
            lm_emb = tf.convert_to_tensor(tokens_lm)
    ...
    def load_eval_data(self):
        if self.eval_data is None:
            ...
            with open(os.path.join(project_dir, self.config["eval_path"])) as f:
    ...
    def evaluate(self, session, official_stdout=False):
        ...
        summary_dict = {}
        conll_results = conll.evaluate_conll(os.path.join(project_dir, self.config["conll_eval_path"]),
                                             coref_predictions, official_stdout)
